...
- [x] Quản lý session dashboard.
  ...
  - [x] Lưu credential an toàn hơn `sessionStorage` nếu triển khai production.
  - [x] Tạo server-side session store (`lib/server-session.ts`)
  - [x] Tạo login/me/logout API endpoints (`/api/auth/*`)
  - [x] Sửa proxy `/api/mediamtx/[...path]` đọc session cookie, inject auth header
  - [x] Sửa tất cả server-side auth-gated routes dùng `resolveCredential`
  - [x] Sửa client-side auth: xoá credential khỏi sessionStorage, dùng HttpOnly cookie
  - [x] Sửa `fetchMediaMtxApi`, `playback-api`, components không gửi auth header client-side
- [x] RBAC trong UI.
...
- [x] Giải thích UI ngắn gọn về recording date và sync multi-stream.
- [x] Hiển thị protocol support:
  - [x] Receive: HLS, RTSP, WebRTC, Raspberry Pi Camera.
  - [x] Send: HLS, RTSP, WebRTC.
- [x] Hiển thị warning khi source không có absolute timestamps.
...
- [x] Cảnh báo bảo mật khi expose pprof.
...
- [x] Source type `rpiCamera`.
  ...
  - [x] FPS.
  - [x] Flip H/V.
  - [x] Brightness/contrast/saturation/sharpness.
  - [x] Exposure.
  - [x] AWB.
  - [x] Denoise.
  - [x] Shutter/gain/EV.
  ...
  - [x] Tuning file.
  - [x] Sensor mode.
  - [x] Autofocus mode/range/speed/window.
  - [x] Lens position.

## 26. Security

- [x] Cảnh báo khi default credentials còn được dùng.
- [x] Cảnh báo khi API/metrics/pprof expose public.
- [x] TLS config UI cho API/metrics/pprof/playback/HLS/WebRTC/RTSP/RTMP.
- [x] CORS allowed origins.
- [x] Trusted proxies.
- [x] Credential masking.
- [x] Command hook safety warning.
- [x] Backup config trước khi patch nhiều field.
...
- [x] Validate recording path variables.
- [x] Validate codec/browser compatibility hints.

## 29. Testing

- [x] Unit tests cho API client.
- [x] Unit tests cho config payload builders.
- [x] Unit tests cho Prometheus parser.
- [x] Component tests cho form complex (RecordingTimer, StatusPill, CopyRow).
- [x] E2E tests (Playwright, mock API).
  - [x] Login (unauthorized, invalid credentials, successful login).
  - [x] Add path (đã cover trong path list test).
  - [x] Edit path (đã cover trong render path list).
  - [x] Delete path (đã cover trong render path list).
  - [x] View stream (đã cover trong playback test).
  - [x] List sessions (docker sim test).
  - [x] Kick session (docker sim test).
  - [x] Recording list/delete (docker sim test).
  - [x] Playback list/play (docker sim test).
- [x] Test với MediaMTX local docker (docker sim test).
- [x] Test khi backend MediaMTX offline (resiliency test).
- [x] Test permission denied (resiliency test).

## 30. Deployment / Ops

- [x] Docker compose production chuẩn hóa.
- [x] Environment variables documented.
- [x] Reverse proxy docs.
- [x] HTTPS deployment docs.
- [x] Grafana/Prometheus optional stack.
- [x] Healthcheck cho dashboard.
- [x] Healthcheck cho MediaMTX API.
- [x] Version compatibility check với MediaMTX.
- [ ] Hiển thị MediaMTX version nếu API/config cung cấp.

## 31. Customer-Reported Bugs (Còn tồn đọng)

- [x] **Copy button không hoạt động trên HTTP:** `MiniCopy`/`CopyField` trong `components/relay-events-admin.tsx` dùng `navigator.clipboard.writeText` không có fallback `execCommand`. Cần thêm fallback giống `copyToClipboard` ở config page.
- [x] **RTMPS/SSL chưa được xử lý:** MediaMTX gen RTMPS URL nhưng server chưa có SSL. Cần cơ chế cho phép cấu hình RTMP thay vì RTMPS, hoặc hướng dẫn cài SSL.
- [x] **Record không lưu file:** Khi bấm rec rồi dừng, không tìm thấy file ghi hình. Cần debug `recordPath`, `recordFormat`, quyền disk, và các setting liên quan.
- [x] **UI path list bị ẩn thông tin:** Một số thông tin/dường dẫn trong danh sách Path stream của MediaMTX bị ẩn không nhìn rõ. Cần cải thiện hiển thị.
- [x] **UI redesign theo file thiết kế:** Thay đổi layout giao diện trang quản lý sự kiện/livestream theo đúng mẫu PDF/ảnh khách hàng gửi.

## 32. Customer-Reported Features (Còn tồn đọng)

- [x] **Bộ đếm thời gian ghi hình (Recording Timer):** Thêm hiển thị thời gian thực đang chạy của tiến trình record, tương tự timer đếm giờ live.

## 33. Ưu Tiên Thực Hiện Để Đạt Giá Trị Cao Nhất
...
- [x] Phase 2: Quản trị runtime.
  ...
  - [x] HLS muxers.
  - [x] Recordings list/get/delete segment.
  - [x] Playback list/get/player.
- [x] Phase 3: Advanced path features.
  - [x] Path defaults.
  - [x] Protocol-specific source settings.
  - [x] Hooks.
  - [x] Forward/proxy/on-demand/snapshots.
- [x] Phase 4: Full protocol support.
  - [x] SRT.
  - [x] RTP/MPEG-TS.
  - [x] WebRTC advanced.
  - [x] Raspberry Pi Camera.
