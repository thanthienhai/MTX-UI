...
const DEFAULT_WEBRTC_BASE_URL = "http://localhost:8889"
...
export function normalizeMediaMtxWebRtcBaseUrl(webRtcUrl) {
  const configuredUrl = cleanBaseUrl(
    webRtcUrl || process.env.NEXT_PUBLIC_MEDIAMTX_WEBRTC_URL,
    DEFAULT_WEBRTC_BASE_URL,
  )
  return withBasePath(configuredUrl)
}

export function buildMediaMtxWebRtcReadUrl(pathName, options = {}) {
  const baseUrl = options.baseUrl || options.webrtcUrl || normalizeMediaMtxWebRtcBaseUrl()
  return `${cleanBaseUrl(baseUrl, DEFAULT_WEBRTC_BASE_URL)}/${encodeURIComponent(normalizePathName(pathName))}/whep`
}

export function buildMediaMtxWebRtcPublishUrl(pathName, options = {}) {
  const baseUrl = options.baseUrl || options.webrtcUrl || normalizeMediaMtxWebRtcBaseUrl()
  return `${cleanBaseUrl(baseUrl, DEFAULT_WEBRTC_BASE_URL)}/${encodeURIComponent(normalizePathName(pathName))}/whip`
}
