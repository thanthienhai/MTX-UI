...
export function getProtocolSummary(protocolCounts = {}, hlsMuxers = []) {
  const rtsp = Number(protocolCounts.rtspConnections || 0) + Number(protocolCounts.rtspSessions || 0)
  const rtmp = Number(protocolCounts.rtmpConnections || 0) + Number(protocolCounts.rtmpsConnections || 0)
  const srt = Number(protocolCounts.srtConnections || 0)
  const webrtc = Number(protocolCounts.webrtcSessions || 0)
  const hls = Number(hlsMuxers.length || 0)
