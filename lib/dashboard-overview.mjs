...
export function buildDashboardOverview({
  ...
  // Configured names exclude the `all_others` catch-all (a template, not a real
  // path). Runtime paths can include streams published under that catch-all and
  // thus absent from the config list, so the honest denominator is the UNION of
  // configured and runtime names — otherwise `readyPaths` can exceed the total
  // and the UI shows nonsense like "2/1".
  const configuredNames = new Set(
    paths.map((p) => p?.name).filter((n) => n && n !== "all_others"),
  )
  const runtimeNames = new Set(
    livePaths.map((p) => p?.name).filter((n) => n && n !== "all_others"),
  )
  const totalPaths = new Set([...configuredNames, ...runtimeNames]).size
  const configuredPaths = configuredNames.size
  ...
  return {
    ...
    streams: {
      ...
      totalPaths,
      readyPaths,
      idlePaths: Math.max(totalPaths - readyPaths, 0),
      totalReaders,
      protocolSummary,
      trafficTotals,
      bitrate,
