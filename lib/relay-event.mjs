...
/**
 * Whether the runOnReady command is an active fan-out (ffmpeg tee) rather than
 * the no-op that only carries metadata. Callers use this to set
 * `runOnReadyRestart`: a live ffmpeg should auto-restart if it dies, but the
 * no-op must NOT restart or MediaMTX re-runs it every few seconds forever.
 */
export function relayHasActiveCommand(meta) {
  if (!meta || meta.relayEnabled === false) return false
  return (meta.destinations || []).some((d) => d && d.enabled)
}
