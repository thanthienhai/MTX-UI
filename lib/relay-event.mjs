...
/** Tag on a fan-out path's runOnReady carrying that destination's metadata. */
export const FANOUT_PREFIX = "# RELAY_FANOUT:"
/** Infix joining an event slug to a destination id to form a fan-out path name. */
export const FANOUT_SEPARATOR = "__fo__"
...
/**
 * The `source` a fan-out path uses: the local RTSP read URL of the ingest path.
 * MediaMTX pulls this so the fan-out path becomes "ready" whenever the ingest
 * is live, which fires its per-destination runOnReady push.
 */
export function fanoutSourceUrl(slug) {
  return localReadUrl(slug)
}

/** Name of the dedicated fan-out path for one destination of an event. */
export function fanoutPathName(slug, destId) {
  return `${slug}${FANOUT_SEPARATOR}${destId}`
}

/** Whether a MediaMTX path name is a fan-out path (vs. an ingest/event path). */
export function isFanoutPathName(name) {
  return String(name || "").includes(FANOUT_SEPARATOR)
}
...
/**
 * Build the ingest path's `runOnReady`: a bare shell no-op carrying the
 * event-level metadata comment. Fan-out no longer lives here — it runs in
 * dedicated per-destination paths (see {@link buildFanoutRunOnReady}) so that
 * editing a destination never reloads the ingest path / kicks the publisher.
 */
export function buildRunOnReady(meta) {
  const comment = `${META_PREFIX}${encodeMeta(meta)}`
  // `sh -c` so a real shell interprets the `:` no-op and the `#` comment
  // (MediaMTX execs runOnReady directly, without a shell).
  return `sh -c ${shellQuote(`: ${comment}`)}`
}

/**
 * The ingest path's runOnReady is always the metadata no-op now, so it must
 * never auto-restart (`runOnReadyRestart: false`). Kept for call-site clarity.
 */
export function relayHasActiveCommand() {
  return false
}

/**
 * Build the `runOnReady` for one destination's dedicated fan-out path. The
 * path's `source` pulls the ingest into MediaMTX under the fan-out path's own
 * name; here we read that local copy (`-c copy`) and push it to the single
 * destination. When the destination is disabled or relay is off we emit a
 * no-op that still carries the destination metadata, so it persists with the
 * path config. The `parentSlug` is embedded so the event can reclaim its
 * destinations by scanning fan-out paths.
 */
export function buildFanoutRunOnReady(dest, { slug, relayEnabled = true } = {}) {
  const stored = { ...dest, parentSlug: slug, v: META_VERSION }
  const comment = `${FANOUT_PREFIX}${encodeMeta(stored)}`
  const active = relayEnabled !== false && !!(dest && dest.enabled)
  let inner
  if (!active) {
    inner = `: ${comment}`
  } else {
    const input = localReadUrl(fanoutPathName(slug, dest.id))
    inner =
      `ffmpeg -nostdin -hide_banner -loglevel warning ` +
      `-timeout 5000000 -stimeout 30000000 -rtsp_transport tcp ` +
      `-i ${shellQuote(input)} -c copy -map 0 ` +
      `-f flv -flvflags no_duration_filesize ` +
      `${shellQuote(destinationUrl(dest))} ${comment}`
  }
  return `sh -c ${shellQuote(inner)}`
}

/**
 * Build the `runOnNotReady` for one destination's fan-out path: the standby
 * slate, self-terminating loop (ffprobes the ingest and exits on reconnect).
 * Returns "" when no standby should run (disabled / relay off / no fallback /
 * unresolvable asset) so the caller clears the field.
 */
export function buildFanoutRunOnNotReady(dest, fallback, { slug, assetBaseUrl, relayEnabled = true } = {}) {
  const active = relayEnabled !== false && !!(dest && dest.enabled)
  if (!active) return ""
  const fb = fallback
  if (!fb || fb.enabled === false || fb.type === "none") return ""
  const teeTargets = `[f=flv:onfail=ignore]${destinationUrl(dest)}`
  const slate = buildSlateArgs(fb, teeTargets, { assetBaseUrl })
  if (!slate) return ""

  const rtsp = localReadUrl(slug)
...
/**
 * Extract a base64url metadata blob tagged by `prefix` from a command string.
 * The command may be wrapped in `sh -c '...'`, so a trailing quote/space can
 * follow the metadata; we take only the base64url token after the prefix.
 * Returns null when the tag is absent or unparseable.
 */
export function parseTaggedMeta(str, prefix) {
  const s = String(str || "")
  const idx = s.indexOf(prefix)
  if (idx === -1) return null
  const encoded = s.slice(idx + prefix.length).match(/^[A-Za-z0-9_-]+/)?.[0]
  if (!encoded) return null
  return decodeMeta(encoded)
}

/**
 * Extract RELAY_META (event-level metadata) from an ingest path's runOnReady.
 * Returns null when the comment is absent or unparseable.
 */
export function parseRunOnReady(runOnReady) {
  return parseTaggedMeta(runOnReady, META_PREFIX)
}

/**
 * Extract RELAY_FANOUT (one destination's metadata, incl. `parentSlug`) from a
 * fan-out path's runOnReady. Returns null when absent or unparseable.
 */
export function parseFanoutMeta(runOnReady) {
  return parseTaggedMeta(runOnReady, FANOUT_PREFIX)
}
...
/**
 * Allowed shape for a user-supplied ingest path (= MediaMTX path name = event
 * slug). Alnum plus `_`/`-` only so it survives URL paths, RTMP app names and
 * SRT streamids without escaping; 3–64 chars.
 */
const CUSTOM_PATH_RE = /^[A-Za-z0-9_-]{3,64}$/

/**
 * Validate an optional user-supplied custom path. Empty/whitespace means "let
 * the system auto-generate" and is OK. Returns `{ ok, value }` on success
 * (value is the trimmed path, or "" to signal auto-gen) or `{ ok:false, error }`.
 */
export function validateCustomPath(value) {
  const v = String(value || "").trim()
  if (!v) return { ok: true, value: "" }
  if (!CUSTOM_PATH_RE.test(v)) {
    return { ok: false, error: "Path chỉ gồm chữ, số, _ và - (3–64 ký tự), không khoảng trắng" }
  }
  // Reserve the fan-out namespace so a custom ingest path can't collide with
  // (or masquerade as) a per-destination fan-out path.
  if (v.includes(FANOUT_SEPARATOR)) {
    return { ok: false, error: `Path không được chứa "${FANOUT_SEPARATOR}"` }
  }
 ...
 *
 * `slug` lets the caller supply a custom (already-validated) ingest path;
 * omit it to auto-generate a random key.
 */
export function createEventMeta({ displayName, quota = 10, loginCode, slug: customSlug } = {}) {
  const slug = customSlug || generateStreamKey()
 ...
 * Destination keys are masked and login-code hash is stripped. Destinations now
 * live in dedicated fan-out paths, so the caller resolves and passes them in.
 */
export function toPublicStatus(meta, destinations = []) {
  if (!meta) return null
  return {
...
export function normalizeDestinationPatch(patch) {
  const out = {}
