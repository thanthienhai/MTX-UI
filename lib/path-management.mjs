...
export function validateDefaultsPatch(patch) {
  ...
  for (const [field, value] of Object.entries(patch)) {
    ...
    if (field === "recordPath" && typeof value === "string" && value.length > 0) {
      // Only token-set violations are hard errors; soft hints (missing %path,
      // missing time variable) are surfaced via getRecordPathTemplateHints.
      const err = validateRecordPathTemplateStrict(value)
...
/**
 * Strict validation: only fails on unknown `%X` tokens. Soft hints
 * (missing %path, missing time variable) are returned by
 * getRecordPathTemplateHints so they don't block save in scenarios
 * where the user knows what they're doing.
 *
 * @param {string} template
 * @returns {string | null}
 */
export function validateRecordPathTemplateStrict(template) {
  if (!template) return null
  const tokens = template.match(/%path|%[A-Za-z]/g) || []
...
/**
 * Returns an array of advisory hints for a record path template.
 * Each hint is informational, not blocking.
 *
 * @param {string} template
 * @returns {string[]}
 */
export function getRecordPathTemplateHints(template) {
  if (!template) return []
  const hints = []
  if (!template.includes("%path")) {
    hints.push("Template không chứa %path — mọi path sẽ ghi vào cùng một thư mục, dễ trộn segment giữa các stream.")
  }
  const hasUnique = template.includes("%f") || template.includes("%S") || template.includes("%M")
  if (!hasUnique) {
    hints.push("Template thiếu biến thời gian (%f, %S, %M). Segment mới có thể ghi đè segment cũ nếu cùng giờ/phút.")
  }
  return hints
}

/**
 * @deprecated Kept for backwards compatibility. Returns the first issue
 * (strict error or first hint). Prefer validateRecordPathTemplateStrict
 * + getRecordPathTemplateHints when you need to distinguish severity.
 *
 * @param {string} template
 * @returns {string | null}
 */
export function validateRecordPathTemplate(template) {
  const strict = validateRecordPathTemplateStrict(template)
  if (strict) return strict
  const hints = getRecordPathTemplateHints(template)
  return hints[0] || null
}
